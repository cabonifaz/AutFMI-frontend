...
import {
  EquipoFormSchema,
  EquipoFormType,
} from "../models/schema/EquipoFormSchema";
...
const PantallaSolicitarEquipo = () => {
  ...
  const onSubmit: SubmitHandler<EquipoFormType> = async (data) => {
    ...
    try {
      const formattedData = {
        ...
        marca: data.marca,
        anexo:
          anexoHardwareParams.find((param) => param.num1 === data.anexoHardware)
            ?.string1 || "",
  ...
  const isLoading =
    postloading || employeeLoading || paramLoading || clientsLoading;
